...
A browser-based version of CITableCleaner lives in the `web/` folder. It runs entirely in the browser — no server, no installation — using [Pyodide](https://pyodide.org) to execute the same pandas-based data logic in a Web Worker.
...
The `web/` folder is self-contained. Copy it to any static hosting service (GitHub Pages, Netlify, Azure Static Web Apps, etc.) and point users at the `index.html` URL. No build step is required.
